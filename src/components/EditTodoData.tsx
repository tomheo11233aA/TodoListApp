...
const EditTodoData: React.FC<ModalProps> = ({ visible, setVisible, setTodos, item }) => {
    ...
    React.useEffect(() => {
        if (item) {
            setTask(item.task);
            setDate(item.date);
            setSelectedOptions(new IndexPath(options.indexOf(item.category)));
        }
    }, [item]);
    ...
    const editTask = () => {
        if (item) {
            ...
            setTodos(oldTodos => oldTodos.map(oldTask => oldTask === item ? newTask : oldTask));
            setTask('');
            setDate(new Date());
    ...
    return (
        <Modal
            ...
            <Card disabled={true}>
                ...
                <Button style={{ backgroundColor: '#50C2C9', width: '90%', alignSelf: 'center' }} onPress={editTask}>
                    Edit Task
                </Button>
            </Card>
        </Modal>
